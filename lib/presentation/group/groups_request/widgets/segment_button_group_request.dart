import 'package:flutter/material.dart';
import 'package:flutter_gen/gen_l10n/app_localizations.dart';

enum Request { sent, received }

class SegmentButtonGroupRequest extends StatefulWidget {
  const SegmentButtonGroupRequest(this.onViewChange, {super.key});

  final Function onViewChange;

  @override
  State<SegmentButtonGroupRequest> createState() =>
      _SegmentButtonGroupRequestState();
}

class _SegmentButtonGroupRequestState extends State<SegmentButtonGroupRequest> {
  Request requestView = Request.sent;

  @override
  Widget build(BuildContext context) {
    return SegmentedButton<Request>(
      style: const ButtonStyle(alignment: AlignmentDirectional.center),
      segments: <ButtonSegment<Request>>[
        ButtonSegment(
            value: Request.sent,
            label: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Text(
                AppLocalizations.of(context)!.requests_sent_text_button_segment,
              ),
            )),
        ButtonSegment(
            value: Request.received,
            label: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Text(
                AppLocalizations.of(context)!
                    .requests_receive_text_button_segment,
              ),
            )),
      ],
      selected: <Request>{requestView},
      onSelectionChanged: (Set<Request> newSelection) {
        setState(() {
          requestView = newSelection.first;
          widget.onViewChange(newSelection.first);
        });
      },
    );
  }
}
