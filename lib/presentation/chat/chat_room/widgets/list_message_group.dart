...
class ListMessageGroup extends StatefulWidget {
  const ListMessageGroup({
    Key? key,
  }) : super(key: key);

  @override
  State<ListMessageGroup> createState() => _ListMessageGroupState();
}

class _ListMessageGroupState extends State<ListMessageGroup> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ChatRoomBloc, ChatRoomState>(
      listener: (context, state) {
        state.whenOrNull(getGroupChatRoomInfoSuccess: (messages, _, isLatest) {
      ...
      builder: (context, state) {
        return state.maybeWhen(
          getGroupChatRoomInfoSuccess: (messages, _, __) {
